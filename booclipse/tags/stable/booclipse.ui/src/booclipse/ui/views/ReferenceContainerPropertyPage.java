...
public class ReferenceContainerPropertyPage extends PreferencePage
	...
	class SelectedReferencesLabelProvider extends ReferenceLabelProvider {		
		public String getColumnText(Object element, int columnIndex) {
			IBooAssemblyReference reference = (IBooAssemblyReference)element;
	...
	public boolean performOk() {
		try {
			IBooAssemblyReference[] references = new IBooAssemblyReference[_references.size()];
			_references.toArray(references);
			IBooAssemblySource assemblySource = getAssemblySource();
	...
	IDoubleClickListener _doubleClickListener = new IDoubleClickListener() {
		public void doubleClick(DoubleClickEvent event) {
			...
			IBooAssemblyReference reference = getReference(element);
		...
		private IBooAssemblyReference getReference(Object element) {
			if (element instanceof IBooAssemblyReference) return (IBooAssemblyReference)element;
			return (IBooAssemblyReference) Platform.getAdapterManager().getAdapter(element, IBooAssemblyReference.class);
		}
	};
	...
	private boolean isValidAssemblySourceReference(IFolder folder) throws CoreException {
		IBooAssemblySource source = BooCore.getAssemblySource(folder);
		return null == source
			? false
	...
	private IBooAssemblySource getAssemblySource() {
		return (IBooAssemblySource)getElement().getAdapter(IBooAssemblySource.class);
	}
