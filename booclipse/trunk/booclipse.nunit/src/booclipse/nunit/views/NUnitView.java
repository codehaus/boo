...
import booclipse.nunit.ITestRunListener;
...
public class NUnitView extends ViewPart {
	...
	class TestListener implements ITestRunListener {
		...
		public void testsStarted(IBooAssemblySource source, final int testCount) {
		...
		public void testsFinished(IBooAssemblySource source) {
			getDisplay().asyncExec(new Runnable() {
				public void run() {
		...
		public void testFailed(IBooAssemblySource source, final String fullName, final String trace) {
			getDisplay().asyncExec(new Runnable() {
				public void run() {
