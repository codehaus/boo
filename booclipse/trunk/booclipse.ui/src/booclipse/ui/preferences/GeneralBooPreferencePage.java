 ...
 */
package booclipse.ui.preferences;
...
public class GeneralBooPreferencePage
	...
	static class RuntimeDirectoryFieldEditor extends DirectoryFieldEditor {
		RuntimeDirectoryFieldEditor(Composite parent) {
			super(BooCore.P_RUNTIME_LOCATION, "&Mono runtime location:", parent);
		...
		protected boolean doCheckState() {
			...
			if (valid) {
				try {
					int status = BooCore.validateRuntimePath(getTextControl().getText());
					return BooCore.RuntimePathStatus.OK == status;
				} catch (IOException e) {
					e.printStackTrace();
	...
	public GeneralBooPreferencePage() {
		super(GRID);
